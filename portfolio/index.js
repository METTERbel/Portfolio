...
const checkLanguageBtn = document.querySelector('.check-language');
...
const changeLanguage = document.querySelectorAll('[data-i18n]');

const checkLanguages = (x) => {
  document.querySelectorAll('.language-item').forEach(a => a.classList.remove('active'));
  x.target.classList.add('active');

  const language = x.target.dataset.language;

  changeLanguage.forEach(phrase => {
    phrase.innerHTML=i18Obj[x.target.innerHTML][phrase.dataset.i18n]
  });
}

checkLanguageBtn.addEventListener('click', checkLanguages);
